...
const getOrderItemUnitInfo = (
  item
) => {
  // const variantInfo =
  //   safeParseVariantInfo(
  //     item.variant_info
  //   );
  const variantInfo =
  safeParseVariantInfo(
    item.variant_info || item.item_data
  );

  const saleMode =
    String(
      item.sale_mode ||
        variantInfo.sale_mode ||
        "piece"
    )
      .trim()
      .toLowerCase();
          ...
          {items.length > 0 && (
            <>
              ...
              <div className="space-y-3">
                ...
                {items.map((item) => {
  const { saleMode, unitName } = getOrderItemUnitInfo(item);
